...
document.addEventListener('readystatechange', () => {
  if (document.readyState === "interactive") apiRef = document.getElementById('api-reference');
})
...
document.addEventListener('keydown', (ev) => {
  ...
  let currSection = apiRef.querySelector("section[style='display: block;']:not(.class-anchor-offset)");
...
function resolveHash(url) {
  ...
  if (obj === apiRef) {
    let firstSection = Array.from(apiRef.children).find((v) => v.tagName === "SECTION");
...
function collapseSections() {
  let sections = Array.from(apiRef.children).filter((el) =>
    el.tagName === "SECTION"
  );
...
function nextClass(sect) {
  return () => {
    ...
    let index = blockedClassIndex === classes.length - 1 ? 0 : blockedClassIndex + 1;
...
function previousClass(sect) {
  return () => {
    ...
    let index = blockedClassIndex === 0 ? classes.length - 1 : blockedClassIndex - 1;
    window.location.hash = classes[index].id;
  }
};
